...
public final class UltraDNSGeoResourceRecordSetApi implements denominator.GeoResourceRecordSetApi {
    ...
    private Iterator<ResourceRecordSet<?>> allByDName(final String dname) {
        return concat(transform(EnumSet.allOf(DirectionalPool.RecordType.class).iterator(),
                new Function<DirectionalPool.RecordType, Iterator<ResourceRecordSet<?>>>() {

                    @Override
                    public Iterator<ResourceRecordSet<?>> apply(DirectionalPool.RecordType input) {
    ...
    public Iterator<ResourceRecordSet<?>> listByNameAndType(String name, final String type) {
        ...
        if ("CNAME".equals(type)) {
            ...
            return filter(
                    concat(iteratorForDNameAndDirectionalType(name, IPV4), iteratorForDNameAndDirectionalType(name, IPV6)),
                    typeEqualTo(type));
        } else if ("A".equals(type) || "AAAA".equals(type)) {
            DirectionalPool.RecordType dirType = "AAAA".equals(type) ? IPV6 : IPV4;
            ...
            return iteratorForDNameAndDirectionalType(name, DirectionalPool.RecordType.valueOf(type));
        }
    }
    ...
    private Iterator<ResourceRecordSet<?>> iteratorForDNameAndDirectionalType(String name,
            DirectionalPool.RecordType dirType) {
    ...
    private static enum IsCNAME implements Predicate<DirectionalPoolRecordDetail> {
        INSTANCE;
